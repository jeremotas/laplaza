extends Node

# ROADMAP A LA DEMO
# [x] Unificar interfaces visuales (REI)
# [x] Armado de sliders con texturas (REI)
# [x] Mejorar texturas de sliders (REI)
# [x] Visual de VICTORIA (REI)
# [x] Visual de DERROTA (REI)
# [ ] Marca de agua de DEMO (REI - JERE - SOLE)
# [x] Implementacion de interfaces (JERE)
# [x] Balanceo de estrategia (SANDO)
# [ ] Revision de stats de unidades (SANDO)
# [x] Textos y localizacion ingles (SOLE)
# [ ] Icono del juego (REI)
# [ ] Testeo de DEMO (TODOS)
# [ ] Revisar situacion de cuenta y pagina de Steam (SOLE)
# [ ] Analizar como se hace para subir una demo (JERE)
# [ ] Buildear y entregar DEMO (JERE)
# [ ] Publicar DEMO (SOLE)
