...
func attack():
	if not inCoolDownAttack:
		...
		if has_node("WeaponPoint") and last_general_direction != Vector2.ZERO:
			...
			b.prepare_explotion(2, Global.settings.patricios.moreno.attack.bullet.explotion.duration, Global.settings.patricios.moreno.attack.bullet.explotion.scale)
